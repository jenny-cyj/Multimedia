...
trained_model = 'retina_lpd/weights/ccpd_custom/mobilenet0.25_epoch_24_ccpd_blue+green+yellow+white_20231101.pth'
cpu = False
...
def load_model(model, pretrained_path, load_to_cpu):
    ...
    if load_to_cpu:
        ...
        if device.type == 'cuda':
            pretrained_dict = torch.load(pretrained_path, map_location=lambda storage, loc: storage.cuda(0), weights_only=True)  # 使用设备索引
        else:
...
if __name__ == '__main__':
    print(extract_retina('.tmp/plate.jpg', '.tmp/plate2.jpg'))