    ...
    def __init__(self, cfg=None, phase='train'):
        ...
        if cfg['name'] == 'mobilenet0.25':
            ...
            if cfg['pretrain']:
                ...
                checkpoint = torch.load("retina_lpd/weights/mobilenetV1X0.25_pretrain.tar", map_location='cpu')

                #print(f"Checkpoint类型: {type(checkpoint)}")
                #print(f"Checkpoint键: {checkpoint.keys() if isinstance(checkpoint, dict) else '不是字典'}")
                ...
                #print(f"State dict第一个元素的设备: {next(iter(state_dict.values())).device}")
